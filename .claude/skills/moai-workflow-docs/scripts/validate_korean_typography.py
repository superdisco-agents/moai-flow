...
class KoreanTypographyValidator:
    ...
    def _validate_file(self, file_path: Path):
        ...
        try:
            ...
            self.results["encoding_issues"].append(
                {"file": str(file_path.relative_to(self.docs_path)), "error": str(e)}
            )
            return
        ...
        for line_no, line in enumerate(lines, 1):
            ...
            if "\u3000" in line:
                file_issues.append({"type": "전각 공백", "line": line_no, "content": line[:80]})
            ...
            for full, half in full_width_parens.items():
                if full in line:
                    file_issues.append({"type": f"전각 문자: {full}", "line": line_no, "content": line[:80]})
    ...
    def _check_punctuation(self, line: str, line_no: int, issues: list):
        ...
        if "。" in line:
            issues.append({"type": "한글 마침표(。) usage", "line": line_no, "content": line[:80]})

        # Korean comma (、) vs English comma (,)
        if "、" in line:
            issues.append({"type": "한글 쉼표(、) usage", "line": line_no, "content": line[:80]})
