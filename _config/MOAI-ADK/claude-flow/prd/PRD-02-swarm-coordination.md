...
MoAI executes agents independently via Task() without coordination infrastructure. Claude-Flow's swarm system enables sophisticated multi-agent coordination with topologies, consensus, and collective intelligence.
...
**Approach**: Use Claude-Flow's existing MCP server for coordination.
...
**Approach**: Use Claude-Flow MCP for coordination, MoAI agents for execution.

```
Claude-Flow MCP:  Coordination, topology, consensus
...
### Phase 1: Add Claude-Flow MCP (Month 1)
...
**Task 3.1**: Evaluate Claude-Flow MCP usage
...
```json
{
  "swarm": {
    "enabled": true,
    "provider": "claude-flow",  // or "native" in future
...
### Phase 1
- [ ] Claude-Flow MCP configured and functional
