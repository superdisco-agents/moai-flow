#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
MoAI-ADK 닷 폴더 클리너
============================

MoAI 프레임워크와 충돌할 수 있는 MoAI가 아닌 .dot 폴더를 검색하고 선택적으로 제거합니다.
필수 프로젝트 폴더는 보존됩니다.

사용법:
    python clean-dot-folders.py [옵션]

옵션:
    --scan-only     삭제 없이 검색 및 보고만 수행
    --force         확인 프롬프트 건너뛰기
    --agent         스웜 조정이 포함된 AI 가이드 모드 활성화
    --path PATH     검색할 사용자 정의 경로 (기본값: 현재 디렉토리)
    --json          결과를 JSON으로 출력
    --verbose       상세 출력 표시

제작: MoAI-ADK Team
라이선스: MIT
"""

import os
import sys
import json
import shutil
import argparse
from pathlib import Path
from typing import Dict, List, Set, Tuple
from datetime import datetime
import subprocess

# ANSI color codes for terminal output
class Colors:
    HEADER = '\033[95m'
    OKBLUE = '\033[94m'
    OKCYAN = '\033[96m'
    OKGREEN = '\033[92m'
    WARNING = '\033[93m'
    FAIL = '\033[91m'
    ENDC = '\033[0m'
    BOLD = '\033[1m'
    UNDERLINE = '\033[4m'

# Whitelist: NEVER remove these folders
WHITELIST: Set[str] = {
    # Git
    '.git',
    '.github',
    '.gitignore',
    '.gitattributes',
    '.gitmodules',

    # Environment
    '.venv',
    '.virtualenv',
    '.env',
    '.env.local',
    '.env.development',
    '.env.production',
    '.env.test',

    # MoAI Core
    '.moai',
    '.mcp.json',

    # IDE
    '.vscode',
    '.idea',
    '.DS_Store',

    # Python
    '.pytest_cache',
    '__pycache__',
    '.mypy_cache',
    '.ruff_cache',
    '.tox',

    # Node
    '.npm',
    '.yarn',
    '.pnpm-store',

    # Build
    '.next',
    '.nuxt',
    '.cache',
    '.parcel-cache',
}

# Known AI framework folders (potential conflicts)
AI_FRAMEWORKS: Set[str] = {
    '.claude-flow',
    '.swarm',
    '.hive-mind',
    '.specstory',
    '.roo',
    '.aider',
    '.cursor',
    '.copilot',
    '.cody',
    '.tabnine',
    '.kite',
}

# MoAI-specific folders to preserve
MOAI_FOLDERS: Set[str] = {
    '.moai',
    '.claude/commands/moai',
}


class DotFolderScanner:
    """충돌 감지 기능이 있는 .dot 폴더 스캐너."""

    def __init__(self, base_path: Path, verbose: bool = False):
        self.base_path = base_path
        self.verbose = verbose
        self.scan_results: Dict[str, Dict] = {}

    def get_folder_size(self, folder_path: Path) -> int:
        """폴더의 총 크기를 바이트 단위로 계산합니다."""
        total_size = 0
        try:
            for dirpath, dirnames, filenames in os.walk(folder_path):
                for filename in filenames:
                    filepath = os.path.join(dirpath, filename)
                    try:
                        total_size += os.path.getsize(filepath)
                    except (OSError, FileNotFoundError):
                        pass
        except (OSError, PermissionError):
            pass
        return total_size

    def format_size(self, size_bytes: int) -> str:
        """바이트를 사람이 읽을 수 있는 크기로 형식화합니다."""
        for unit in ['B', 'KB', 'MB', 'GB']:
            if size_bytes < 1024.0:
                return f"{size_bytes:.2f} {unit}"
            size_bytes /= 1024.0
        return f"{size_bytes:.2f} TB"

    def is_moai_folder(self, folder_name: str, folder_path: Path) -> bool:
        """폴더가 MoAI 전용인지 확인합니다."""
        if folder_name in MOAI_FOLDERS:
            return True

        # Check for .claude/commands/moai path
        if '.claude' in str(folder_path) and 'moai' in str(folder_path).lower():
            return True

        return False

    def classify_folder(self, folder_name: str, folder_path: Path) -> str:
        """폴더를 카테고리로 분류합니다."""
        if folder_name in WHITELIST:
            return 'whitelisted'

        if self.is_moai_folder(folder_name, folder_path):
            return 'moai'

        if folder_name in AI_FRAMEWORKS:
            return 'ai_framework'

        return 'unknown'

    def scan(self) -> Dict[str, List[Dict]]:
        """.dot 폴더를 검색하고 분류합니다."""
        results = {
            'whitelisted': [],
            'moai': [],
            'ai_framework': [],
            'unknown': [],
        }

        try:
            # Scan immediate .dot folders
            for item in self.base_path.iterdir():
                if item.is_dir() and item.name.startswith('.'):
                    folder_name = item.name
                    folder_size = self.get_folder_size(item)
                    category = self.classify_folder(folder_name, item)

                    folder_info = {
                        'name': folder_name,
                        'path': str(item.absolute()),
                        'size': folder_size,
                        'size_formatted': self.format_size(folder_size),
                        'category': category,
                    }

                    results[category].append(folder_info)

                    if self.verbose:
                        self._print_folder_info(folder_info)

            # Also check for nested .claude/commands/moai
            claude_path = self.base_path / '.claude' / 'commands' / 'moai'
            if claude_path.exists() and claude_path.is_dir():
                folder_size = self.get_folder_size(claude_path)
                folder_info = {
                    'name': '.claude/commands/moai',
                    'path': str(claude_path.absolute()),
                    'size': folder_size,
                    'size_formatted': self.format_size(folder_size),
                    'category': 'moai',
                }
                # Check if not already added
                if not any(f['path'] == folder_info['path'] for f in results['moai']):
                    results['moai'].append(folder_info)

        except PermissionError as e:
            print(f"{Colors.FAIL}권한이 거부되었습니다: {e}{Colors.ENDC}")

        self.scan_results = results
        return results

    def _print_folder_info(self, folder_info: Dict) -> None:
        """색상으로 폴더 정보를 출력합니다."""
        category = folder_info['category']
        color_map = {
            'whitelisted': Colors.OKGREEN,
            'moai': Colors.OKCYAN,
            'ai_framework': Colors.WARNING,
            'unknown': Colors.FAIL,
        }
        color = color_map.get(category, Colors.ENDC)

        print(f"{color}  {folder_info['name']:<30} {folder_info['size_formatted']:>10} [{category}]{Colors.ENDC}")


class DotFolderCleaner:
    """MoAI가 아닌 .dot 폴더를 제거하는 클리너."""

    def __init__(self, scanner: DotFolderScanner, force: bool = False):
        self.scanner = scanner
        self.force = force
        self.deleted_folders: List[Dict] = []
        self.errors: List[Dict] = []

    def get_deletable_folders(self) -> List[Dict]:
        """안전하게 삭제할 수 있는 폴더 목록을 가져옵니다."""
        deletable = []

        # AI framework folders are safe to delete
        deletable.extend(self.scanner.scan_results['ai_framework'])

        # Unknown folders can be deleted with confirmation
        deletable.extend(self.scanner.scan_results['unknown'])

        return deletable

    def confirm_deletion(self, folders: List[Dict]) -> bool:
        """사용자에게 삭제를 확인합니다."""
        if self.force:
            return True

        print(f"\n{Colors.WARNING}다음 폴더가 삭제됩니다:{Colors.ENDC}")
        total_size = 0

        for folder in folders:
            print(f"  {Colors.BOLD}{folder['name']}{Colors.ENDC} ({folder['size_formatted']})")
            total_size += folder['size']

        print(f"\n{Colors.BOLD}확보될 총 크기: {self.scanner.format_size(total_size)}{Colors.ENDC}")

        response = input(f"\n{Colors.WARNING}삭제를 계속하시겠습니까? [y/N]: {Colors.ENDC}")
        return response.lower() in ('y', 'yes', '예', 'ㅇ')

    def delete_folder(self, folder_info: Dict) -> bool:
        """단일 폴더를 삭제합니다."""
        try:
            folder_path = Path(folder_info['path'])

            if folder_path.exists():
                shutil.rmtree(folder_path)
                self.deleted_folders.append(folder_info)
                print(f"{Colors.OKGREEN}✓ 삭제됨: {folder_info['name']}{Colors.ENDC}")
                return True
            else:
                print(f"{Colors.WARNING}⚠ 찾을 수 없음: {folder_info['name']}{Colors.ENDC}")
                return False

        except Exception as e:
            error_info = {
                'folder': folder_info['name'],
                'error': str(e),
            }
            self.errors.append(error_info)
            print(f"{Colors.FAIL}✗ 삭제 실패 {folder_info['name']}: {e}{Colors.ENDC}")
            return False

    def clean(self) -> Dict:
        """삭제 가능한 폴더를 정리합니다."""
        deletable = self.get_deletable_folders()

        if not deletable:
            print(f"{Colors.OKGREEN}정리할 폴더가 없습니다!{Colors.ENDC}")
            return {
                'deleted': [],
                'errors': [],
                'total_freed': 0,
            }

        if not self.confirm_deletion(deletable):
            print(f"{Colors.WARNING}삭제가 취소되었습니다.{Colors.ENDC}")
            return {
                'deleted': [],
                'errors': [],
                'total_freed': 0,
                'cancelled': True,
            }

        print(f"\n{Colors.HEADER}삭제 시작...{Colors.ENDC}")

        for folder in deletable:
            self.delete_folder(folder)

        total_freed = sum(f['size'] for f in self.deleted_folders)

        return {
            'deleted': self.deleted_folders,
            'errors': self.errors,
            'total_freed': total_freed,
            'total_freed_formatted': self.scanner.format_size(total_freed),
        }


class AIGuidedMode:
    """스웜 조정이 포함된 AI 가이드 모드."""

    def __init__(self, scanner: DotFolderScanner):
        self.scanner = scanner

    def analyze_with_swarm(self) -> Dict:
        """AI 스웜을 사용하여 폴더를 분석하고 작업을 권장합니다."""
        print(f"{Colors.HEADER}AI 가이드 모드: 스웜으로 분석 중...{Colors.ENDC}")

        try:
            # Initialize swarm
            swarm_cmd = [
                'npx', 'claude-flow@alpha', 'swarm', 'init',
                '--topology', 'mesh',
                '--max-agents', '3'
            ]

            result = subprocess.run(swarm_cmd, capture_output=True, text=True, timeout=30)

            if result.returncode == 0:
                print(f"{Colors.OKGREEN}✓ 스웜이 초기화되었습니다{Colors.ENDC}")
            else:
                print(f"{Colors.WARNING}⚠ 스웜 초기화를 건너뛰었습니다{Colors.ENDC}")

        except Exception as e:
            print(f"{Colors.WARNING}⚠ AI 모드를 사용할 수 없습니다: {e}{Colors.ENDC}")

        # Return analysis recommendations
        deletable = []
        deletable.extend(self.scanner.scan_results['ai_framework'])

        return {
            'recommended_deletions': deletable,
            'ai_analysis': 'MoAI와 충돌할 수 있는 AI 프레임워크 폴더가 감지되었습니다',
            'confidence': '높음',
        }


def print_report(scan_results: Dict, verbose: bool = False) -> None:
    """검색 보고서를 출력합니다."""
    print(f"\n{Colors.HEADER}{Colors.BOLD}=== MoAI 닷 폴더 검색 보고서 ==={Colors.ENDC}\n")

    # Summary
    total_folders = sum(len(folders) for folders in scan_results.values())
    print(f"발견된 총 .dot 폴더: {Colors.BOLD}{total_folders}{Colors.ENDC}")

    # Whitelisted
    whitelisted = scan_results['whitelisted']
    print(f"\n{Colors.OKGREEN}화이트리스트 (보호됨):{Colors.ENDC} {len(whitelisted)}")
    if verbose:
        for folder in whitelisted:
            print(f"  {folder['name']} ({folder['size_formatted']})")

    # MoAI
    moai = scan_results['moai']
    print(f"\n{Colors.OKCYAN}MoAI 폴더 (보호됨):{Colors.ENDC} {len(moai)}")
    if verbose or moai:
        for folder in moai:
            print(f"  {folder['name']} ({folder['size_formatted']})")

    # AI Frameworks
    ai_frameworks = scan_results['ai_framework']
    if ai_frameworks:
        print(f"\n{Colors.WARNING}AI 프레임워크 충돌:{Colors.ENDC} {len(ai_frameworks)}")
        for folder in ai_frameworks:
            print(f"  {Colors.WARNING}{folder['name']}{Colors.ENDC} ({folder['size_formatted']})")

    # Unknown
    unknown = scan_results['unknown']
    if unknown:
        print(f"\n{Colors.FAIL}알 수 없는 .dot 폴더:{Colors.ENDC} {len(unknown)}")
        for folder in unknown:
            print(f"  {folder['name']} ({folder['size_formatted']})")


def main():
    """메인 진입점."""
    parser = argparse.ArgumentParser(
        description='MoAI-ADK 닷 폴더 클리너',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )

    parser.add_argument('--scan-only', action='store_true',
                       help='삭제 없이 검색 및 보고만 수행')
    parser.add_argument('--force', action='store_true',
                       help='확인 프롬프트 건너뛰기')
    parser.add_argument('--agent', action='store_true',
                       help='스웜 조정이 포함된 AI 가이드 모드 활성화')
    parser.add_argument('--path', type=str, default='.',
                       help='검색할 사용자 정의 경로 (기본값: 현재 디렉토리)')
    parser.add_argument('--json', action='store_true',
                       help='결과를 JSON으로 출력')
    parser.add_argument('--verbose', action='store_true',
                       help='상세 출력 표시')

    args = parser.parse_args()

    # Initialize scanner
    base_path = Path(args.path).resolve()

    if not base_path.exists():
        print(f"{Colors.FAIL}오류: 경로가 존재하지 않습니다: {base_path}{Colors.ENDC}")
        sys.exit(1)

    print(f"{Colors.HEADER}검색 중: {base_path}{Colors.ENDC}\n")

    scanner = DotFolderScanner(base_path, verbose=args.verbose)
    results = scanner.scan()

    # Print report
    if not args.json:
        print_report(results, verbose=args.verbose)

    # AI-guided mode
    if args.agent:
        ai_mode = AIGuidedMode(scanner)
        ai_results = ai_mode.analyze_with_swarm()

        if not args.json:
            print(f"\n{Colors.OKCYAN}AI 분석:{Colors.ENDC} {ai_results['ai_analysis']}")
            print(f"{Colors.OKCYAN}신뢰도:{Colors.ENDC} {ai_results['confidence']}")

    # Scan-only mode
    if args.scan_only:
        if args.json:
            print(json.dumps(results, indent=2))
        else:
            print(f"\n{Colors.OKBLUE}검색이 완료되었습니다. 정리하려면 --scan-only 없이 사용하세요.{Colors.ENDC}")
        sys.exit(0)

    # Clean mode
    cleaner = DotFolderCleaner(scanner, force=args.force)
    clean_results = cleaner.clean()

    # Output results
    if args.json:
        output = {
            'scan_results': results,
            'clean_results': clean_results,
            'timestamp': datetime.now().isoformat(),
        }
        print(json.dumps(output, indent=2))
    else:
        if clean_results.get('deleted'):
            print(f"\n{Colors.OKGREEN}{Colors.BOLD}✓ 정리가 완료되었습니다!{Colors.ENDC}")
            print(f"{len(clean_results['deleted'])}개의 폴더가 삭제되었습니다")
            print(f"{clean_results['total_freed_formatted']} 확보되었습니다")

        if clean_results.get('errors'):
            print(f"\n{Colors.FAIL}발생한 오류:{Colors.ENDC}")
            for error in clean_results['errors']:
                print(f"  {error['folder']}: {error['error']}")


if __name__ == '__main__':
    main()
