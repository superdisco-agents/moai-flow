...
MoAI-Flow makes a **critical distinction** between MCP tools and Claude Code's Task tool. This is one of the most important architectural concepts in MoAI-Flow.
...
```javascript
mcp__moai-flow__swarm_init { topology: "mesh" }
mcp__moai-flow__agent_spawn { type: "researcher" }
mcp__moai-flow__agent_spawn { type: "coder" }
```
