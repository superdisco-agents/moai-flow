...
> MoAI-Flow CLI commands for SPARC methodology execution

## Overview

MoAI-Flow provides a dedicated CLI (`npx moai-flow sparc`) for executing SPARC methodology phases.
...
```bash
npx moai-flow sparc run <mode> "<task>"
```

**Examples**:
```bash
npx moai-flow sparc run spec-pseudocode "Design user authentication system"
npx moai-flow sparc run architect "Design microservices architecture"
npx moai-flow sparc run integration "Integrate payment gateway"
```

### TDD Workflow

```bash
npx moai-flow sparc tdd "<feature>"
```
...
**Example**:
```bash
npx moai-flow sparc tdd "User registration with email verification"
```

### Get Mode Information

```bash
npx moai-flow sparc info <mode>
```
...
```bash
npx moai-flow sparc batch <modes> "<task>"
```
...
**Example**:
```bash
npx moai-flow sparc batch "spec-pseudocode,architect" "Design API gateway"
```

### Full Pipeline

```bash
npx moai-flow sparc pipeline "<task>"
```
...
```bash
npx moai-flow sparc concurrent <mode> "<tasks-file>"
```
...
|---------|-------------|------|
| CLI Tool | `npx moai-flow sparc` | Slash commands |
...
### MoAI-Flow Full Workflow

```bash
# Step 1: Specification
npx moai-flow sparc run spec-pseudocode "Build REST API for user management"

# Step 2: Architecture
npx moai-flow sparc run architect "Design service architecture"

# Step 3: TDD Implementation
npx moai-flow sparc tdd "User CRUD operations"

# Step 4: Integration
npx moai-flow sparc run integration "Connect to frontend"
```
...
MoAI-Flow has a richer CLI experience, while MoAI uses slash commands within Claude Code. Both achieve similar outcomes through different interfaces.
