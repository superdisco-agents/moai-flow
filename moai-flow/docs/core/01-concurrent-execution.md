...
MoAI-Flow enforces strict concurrent execution patterns to maximize efficiency and reduce token usage by 32.3%.
...
**MoAI-Flow Rule**: ALWAYS batch ALL todos in ONE call (5-10+ todos minimum)
...
**MoAI-Flow Rule**: ALWAYS spawn ALL agents in ONE message with full instructions
...
**MoAI-Flow Rule**: ALWAYS batch ALL reads/writes/edits in ONE message
...
**MoAI-Flow Rule**: ALWAYS batch ALL terminal operations in ONE message
...
**MoAI-Flow Rule**: ALWAYS batch ALL memory store/retrieve in ONE message

```javascript
// ✅ CORRECT: All memory ops together
[Single Message]:
  mcp__moai-flow__memory_store { key: "api/schema", value: {...} }
  mcp__moai-flow__memory_store { key: "api/endpoints", value: {...} }
  mcp__moai-flow__memory_retrieve { key: "api/config" }
```
