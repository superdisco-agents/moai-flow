...
> Organization of MoAI-Flow (native) tools by function

## Overview

MoAI-Flow organizes MCP tools into distinct categories based on their purpose. This helps understand when to use each tool.
...
mcp__moai-flow__agent_spawn { type: "researcher" }
mcp__moai-flow__agent_spawn { type: "coder" }
...
// List active agents
mcp__moai-flow__agent_list {}

// Get agent performance
mcp__moai-flow__agent_metrics { agentId: "researcher-1" }

// Check task progress
mcp__moai-flow__task_status { taskId: "task-123" }
```
...
mcp__moai-flow__features_detect {}
```
...
| MoAI-Flow Category | MoAI Equivalent |
|---------------------|-----------------|
| Coordination | None |
| Monitoring | None |
| Memory & Neural | None |
...
MoAI-Flow: More coordination-focused
