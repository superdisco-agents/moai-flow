#!/usr/bin/env python3
"""
Comprehensive markdown validation for Korean documentation.

Usage:
    uv run lint_korean_docs.py [--input PATH] [--output PATH] [--json]
    uv run lint_korean_docs.py --help
...
class KoreanDocsLinter:
    ...
    def lint_file(self, file_path: Path):
        ...
        try:
            ...
            self.errors.append(
                {"file": file_path, "line": "N/A", "type": "file", "message": f"File read error: {str(e)}"}
            )
    ...
    def validate_mermaid_content(self, file_path, line_no, content):
        ...
        if not content.strip():
            self.errors.append(
                {"file": file_path, "line": line_no, "type": "mermaid", "message": "Empty Mermaid block"}
            )
            return
    ...
    def check_korean_specifics(self, file_path, content):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if "\u3000" in line:
                self.warnings.append(
                    {"file": file_path, "line": i, "type": "korean", "message": "전각 공백 (U+3000) detected"}
                )

            # Full-width parentheses (Korean validation rule - PRESERVE)
            if "（" in line or "）" in line:
                self.info.append(
                    {"file": file_path, "line": i, "type": "korean", "message": "전각 괄호 usage detected"}
                )

            # Full-width double quotes (Korean validation rule - PRESERVE)
            if '"' in line or '"' in line:
                self.info.append(
                    {"file": file_path, "line": i, "type": "korean", "message": "전각 쌍따옴표 usage detected"}
                )
    ...
    def check_whitespace(self, file_path, content):
        ...
        if content and not content.endswith("\n"):
            self.info.append(
                {"file": file_path, "line": "EOF", "type": "whitespace", "message": "No newline at end of file"}
            )

    def generate_report(self, as_json: bool = False):
        """Generate validation report in human or JSON format"""
        if as_json:
...
def main():
    ...
    @click.command()
    @click.option('--input', type=click.Path(exists=True), default=None,
                  help='Input path (file or directory). Defaults to current directory.')
    ...
    def cli(input, output, output_json):
        ...
        # Run validation
        linter = KoreanDocsLinter(str(input_path))
        linter.lint_all()

        # Generate report
        report_text = linter.generate_report(as_json=output_json)
