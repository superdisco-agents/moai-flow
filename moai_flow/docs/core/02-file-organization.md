...
> MoAI-Flow's Absolute Rule: NEVER save working files to the root folder

## Overview

MoAI-Flow enforces strict file organization to prevent project root pollution - a principle MoAI already partially implements via `.moai/` hierarchy.
...
MoAI's existing system is **superior** to MoAI-Flow's simple rules.

**Enhancement opportunity**: Make `block_root_pollution: true` the default for stricter enforcement matching MoAI-Flow's philosophy.
